...
def test_groq() -> tuple[bool, str]:
    try:
        ...
        if result and result.get("title"):
            return True, f"✅ Groq AI — '{result['title'][:60]}'"
        return False, "❌ Groq вернул пустой результат"
    except Exception as e:
        return False, f"❌ {e}"
...
async def run_full_selftest(bot, admin_id: str, scheduler=None) -> str:
    ...
    # 3. AI
    all_results.append(("", None, "─── 3. GROQ AI ───"))
    ok_ai, det_ai = await asyncio.to_thread(test_groq)
    all_results.append(("Groq AI analyzer", ok_ai, det_ai))
